...
| ⬜ open | low | S | Artwork placeholder is a blank grey rectangle, not a recognizable icon, for both loading and failed states | User-observed 2026-06-30: tracks with no/failed artwork render as blank grey with no affordance. AsyncCachedImage's placeholder is caller-supplied as a plain fill — MiniPlayerView (Rectangle().fill(.gray.opacity(0.3))), FullScreenPlayerView (Rectangle().fill(themeManager.dividerColor)), TrackThumbnail, library/playlist rows — none show a music-note glyph; no shared ArtworkPlaceholder view exists. |
