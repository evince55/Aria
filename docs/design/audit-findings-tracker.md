...
| ⬜ open | medium | M | Local/offline tracks show no artwork — embedded cover art is not reliably extracted | Imported local files render placeholder artwork in app (user-observed 2026-06-30). extractArtwork(from:trackID:) (LocalLibraryManager.swift:352) feeds LocalTrack.artworkURL→thumbnailURL (LocalTrack.swift:55), so the display path is wired; extraction returns nil for common formats (likely only AVMetadata commonKey/`.artwork` handled, missing ID3/FLAC/Vorbis-comment pictures and async/format edge cases). Investigate + broaden extraction; add coverage. |
