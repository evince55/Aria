...
| ✅ done | high | M | Search is capped at 25 results with no pagination or infinite scroll | #16: backend `/api/search` takes `limit` (1–50) + `offset`, fetching `ytsearch{offset+limit}` and slicing the tail, capped at `SEARCH_FETCH_MAX=100`; `MAX_QUERY_LEN=200`. iOS SearchView infinite-scrolls via last-row onAppear→loadMore with id-dedupe, a self-animating bottom spinner (shown while `canLoadMore`), and surfaces track duration (`m:ss`). |
...
| ✅ done | medium | M | Add a cover-art lookup endpoint so offline/local tracks with no embedded art can be populated | Added `GET /api/cover?title=&artist=&duration=` (app.py). Primary source: iTunes Search API (`_itunes_search_sync`, no key, `entity=song&limit=5`), best match picked by casefolded artist-name match then closest `trackTimeMillis` to the supplied `duration` (breaks ties between live/studio/wrong releases), `artworkUrl100` upscaled to `600x600bb.jpg` via regex rewrite (`_upscale_itunes_artwork`). Fallback: `_youtube_thumbnail_sync` reuses the `ytsearch1:`/`extract_flat` yt-dlp path already used by `/api/search` and returns the top hit's thumbnail. Response shape is always 200 `{"cover_url": <url>|null, "source": "itunes"|"youtube"|null}` — never 404/500, so upstream/network/JSON failures degrade to null (caught per-source in `_lookup_cover_sync`). Cost controls: in-memory `_cover_cache` keyed by normalized+5s-bucketed `(title, artist, duration)` with a 7-day TTL (`COVER_CACHE_TTL`) and `COVER_CACHE_MAX_KEYS`-bounded eviction (`_prune_cover_cache`, TTL-expired first then oldest-inserted) mirroring the search-cache/rate-limit-table patterns; new `RATE_LIMIT_COVER_PER_MIN` (default 30) reuses `_enforce_rate_limit`; runs under the existing `_ytdl_search_semaphore` (cheap, no download slot). Outbound iTunes call uses stdlib `urllib.request` with a `COVER_HTTP_TIMEOUT_SECONDS` (default 5) timeout — no new dependency. Opt-in `X-API-Key` auth applied via the existing `_require_api_key` dependency, consistent with other endpoints. Covered by 8 new pytest cases in `backend/tests/test_app.py` (iTunes hit + upscale, duration tie-break, iTunes miss → YouTube fallback, both-miss → null, malformed-JSON and network-error graceful degradation, cache-hit avoids second outbound call, rate-limit 429). Full backend suite: 67 passed. iOS wiring is a separate data-lane follow-up (not in this PR). |
...
| ✅ done | medium | M | Wire local tracks to the /api/cover backend so tracks with no embedded art get a web cover | Added `CoverFetching` protocol + `BackendCoverFetcher` (default impl) in `Managers/LocalLibraryManager.swift`: builds `GET {PlayerManager.backendURL}/api/cover?title=&artist=&duration=` (percent-encoded via `URLComponents`, artist omitted when nil/empty/"This Device", duration omitted when absent/0), attaches the opt-in `X-API-Key` header from `PlayerManager.apiKey` (read-only reuse, no PlayerManager edits), parses `{cover_url, source}`, downloads the image bytes, and writes them via the existing `writeArtwork`/`AtomicFileWriter` + magic-byte-extension path shared with embedded extraction. Wired at two call sites: (1) `importFile` — after `extractArtwork` returns nil; (2) new `backfillRemoteCovers()` (sibling to `healMissingArtwork()`, runs on init) which fetches for any existing track with no artwork file on disk, guarded by a per-launch `attemptedRemoteCoverTrackIDs` set so it fetches at most once per track per launch (a miss is retried on the next launch, never looped). Everything best-effort — network/decode/timeout failures return nil and leave the track coverless, never throw, 8s timeout. `LocalTrack`/schema unchanged (reuses `artworkFileName`). Tests added to `Tests/LocalLibraryManagerTests.swift` via an injected `CoverFetching` mock: remote fetch fills a coverless track and writes the file (`test_import_noEmbeddedArtwork_fetchesRemoteCover_andWritesFile`), nil/failing fetcher leaves the track coverless without crashing (`test_import_remoteCoverFetcher_returnsNil_leavesTrackCoverless_noCrash`, `test_import_remoteCoverFetcher_failure_leavesTrackCoverless_noCrash`), embedded artwork short-circuits the remote call and is never overwritten (`test_import_embeddedArtworkPresent_doesNotCallRemoteFetcher_notOverwritten`), and the backfill pass covers an existing coverless track getting fetched on next init plus an already-covered track being left untouched (`test_backfillRemoteCovers_existingCoverlessTrack_getsRemoteCoverOnNextInit`, `test_backfillRemoteCovers_trackAlreadyHasArtwork_isNotOverwritten`). Full `AriaTests` suite: 246 tests, 0 failures (2 pre-existing live-integration tests skipped, unrelated to this change). |
