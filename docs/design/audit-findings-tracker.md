...
| ⬜ open | medium | M | Add a cover-art lookup endpoint so offline/local tracks with no embedded art can be populated | Feature request 2026-07-01: local-library tracks whose embedded art is nil currently fall back to the music-note placeholder (#17). Backend has no cover endpoint (grep app.py: none). Add `GET /api/cover?title=&artist=&duration=` → query iTunes Search API (square album art, no key), fall back to a YouTube-thumbnail match, return best image URL; cache + rate-limit like other endpoints. iOS wiring is a separate data-lane follow-up. |
